...
private let RemoteTypeEnvKey = "MusicRemote"
...
///
/// The change processors (`ChangeProcessor`) each have 1 specific aspect of syncing as their role. In our case there are three: one for uploading songs, one for downloading songs, and one for removing songs.
///
...
extension SyncCoordinator {
    ...
    fileprivate func fetchNewRemoteData() {
        remote.fetchNewSongs { changes, callback in
