...
struct ConversationStatus: OptionSet {
    let rawValue: Int32
    
    static let common       = ConversationStatus(rawValue: 1 << 0)
    static let topChat      = ConversationStatus(rawValue: 1 << 1)
    static let deleted      = ConversationStatus(rawValue: 1 << 2)
    static let interruption = ConversationStatus(rawValue: 1 << 3)
}
...
extension Conversation: DisplayableManaged {
    ...
    var subTitle_: String? {
        return String(NSString.timeChatListHourAndSeconds(TimeInterval(lastMessageTime)))
    }
