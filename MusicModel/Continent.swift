...
public class Continent: NSManagedObject {
    ...
    func updateSongCount() {
        ...
        let deltaInCountries: Int64 = currentAndDeletedCountries.reduce(0) { $0 + $1.changedSongCountDelta }
        let pendingDelta = numberOfSongs - committedNumberOfSongs
        guard pendingDelta != deltaInCountries else { return }
        numberOfSongs = committedNumberOfSongs + deltaInCountries
    }
    ...
    fileprivate var committedNumberOfSongs: Int64 {
        let n = committedValue(forKey: #keyPath(Continent.numberOfSongs)) as? Int ?? 0
        return Int64(n)
    }

    fileprivate var hasChangedNumberOfSongs: Bool {
        return changedValue(forKey: #keyPath(Continent.numberOfSongs)) != nil
    }
